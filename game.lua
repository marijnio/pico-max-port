...
function draw_game()
  ...
  -- group all characters by tile position
  for c in all(game.chars) do
  ...
  for pos, chars_on_tile in pairs(chars_by_pos) do
    ...
    for i=1,min(#chars_on_tile, 4) do
      ...
      -- caught critters get a separate color
      local col = c.caught and 2 or (c.can_move and 11 or 8)

      -- 3x3 corner marker inside the tile
      rectfill(px, py, px+2, py+2, col)

      -- add a tiny mark to indicate caught state
      if c.caught then
        pset(px+1, py+1, 7)
      end

      -- show exactly which character is selected
      if c == selected then
